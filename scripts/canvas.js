...
// Define the pan boundaries (for example)
var panLimits = {
  left: -200,    // Max pan left
  right: 200,    // Max pan right
  top: -100,     // Max pan up
  bottom: 100    // Max pan down
};

// Function to limit the pan
function limitPan() {
  // Get the current viewport transformation
  var vpt = canvas.viewportTransform;
  
  // Limit the panning on the x-axis
  if (vpt[4] > panLimits.right) {
    vpt[4] = panLimits.right;
  } else if (vpt[4] < panLimits.left) {
    vpt[4] = panLimits.left;
  }
  
  // Limit the panning on the y-axis
  if (vpt[5] > panLimits.bottom) {
    vpt[5] = panLimits.bottom;
  } else if (vpt[5] < panLimits.top) {
    vpt[5] = panLimits.top;
  }
  
  // Apply the constrained viewport transformation
  canvas.setViewportTransform(vpt);
}

// Listen for the 'mouse:move' event to adjust panning
canvas.on('mouse:move', function(options) {
  if (options.e.buttons !== 1) return; // Only pan when the left mouse button is pressed
  
  limitPan();
});

// Enable dragging on the canvas
...
canvas.on('mouse:move', function(options) {
  ...
  vpt[4] += options.e.clientX - canvas.lastPosX;
  vpt[5] += options.e.clientY - canvas.lastPosY;
  
  limitPan(); // Apply the pan limits
